...
var Main = (function (_super) {
    ...
    * configuration file loading is completed, start to pre-load the preload resource group
    */
    p.onResourceBeiginLoadComplete = function (event) {
     ...
     *
     */
    p.onResourceMidComplete = function (event) {
    ...
    p.createGameScene = function () {
        ...
        var myscrollView = new egret.ScrollView(); //创建滚动窗口
        var draggedObject;
        var offsetY;
        for (var i = 0; i < 20; i++) {
            var map = 'map_' + (i + 1) + '_jpg';
        ...
        //声音的小图标
        var voice = this.createBitmapByName('voice_png');
     ...
     *
     */
    p.createaloneBItmap = function (name, x, y, scaleX, scaleY, tween, endX, endY, rotation) {
        var bomImg = this.createBitmapByName(name);
        bomImg.x = x;
        bomImg.y = y;
     ...
     * Create a Bitmap object according to name keyword.As for the property of name please refer to the configuration file of resources/resource.json.
     */
    p.createbg = function (name, num, container) {
        var bomImg = this.createBitmapByName(name);
        container.addChild(bomImg);
        var stageW = Math.floor(this.stage.stageWidth / 2);
        var stageH = Math.floor(this.stage.stageWidth / 2 * 544 / 375);
    ...
    p.createLantern = function (month, day, color) {
        var lancon = new egret.Sprite();
        var lant = this.createBitmapByName(color);
    ...
    //创建灯笼
    p.creatLan = function (month, day, color, container, num) {
    ...
    ////创建签到的按钮
    p.createSign = function (text, name) {
        var box = new egret.Sprite();
        container.addChild(box);
        box.x = 505;
        box.y = 80;
        var signImg = this.createBitmapByName(name);
    ...
    ////创建气球运动
    p.createballon = function (textHead, textBot) {
        var ballon = new egret.Sprite();
        container.addChild(ballon);
        ballon.x = 0;
        ballon.y = 2030;
        //氢气球
        var ballonImg = this.createBitmapByName('balloon_png');
     ...
     * scaleX,scaleY,[0-不缩放]；
     */
    p.frametween = function (jsonname, imgname, name, x, y, frame, rescord, scaleX, scaleY) {
        var data = RES.getRes(jsonname);
        var txtr = RES.getRes(imgname);
        var mcFactory = new egret.MovieClipDataFactory(data, txtr);
        var mc1 = new egret.MovieClip(mcFactory.generateMovieClipData(name));
        mc1.x = x;
        mc1.y = y;
     ...
     */
    p.mantween = function (jsonname, imgname, name, x, y, frame, rescord, endX, endY, interval) {
        var data = RES.getRes(jsonname);
        var txtr = RES.getRes(imgname);
        var mcFactory = new egret.MovieClipDataFactory(data, txtr);
        var mc1 = new egret.MovieClip(mcFactory.generateMovieClipData(name));
     ...
     * interval:移动到目标位置需要的时间
     */
    p.cartween = function (jsonname, imgname, name, x, y, frame, rescord, endX, endY, interval) {
        var data = RES.getRes(jsonname);
        var txtr = RES.getRes(imgname);
        var mcFactory = new egret.MovieClipDataFactory(data, txtr);
        var mc1 = new egret.MovieClip(mcFactory.generateMovieClipData(name));
